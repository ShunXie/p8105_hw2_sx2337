...
# Problem 2
First, load the data. The first line of the data is a logo, in which need to skip it. Also do not include the last several columns by specifying the columns that need to be included in the range argument in read_excel. After clean the names, change the variable name of weight_tons to weight and Volume_cubic_yards to volume. After that, ommit rows that does not include dumpster-specific data using drop_na function. The last row contains grand total will also be excluded by using filter function filters all rows except for the last row. Lastly, round the number of sports balls to the nearest integer and converts the result to an integer variable.
```{r}
Mr_trash_wheel = 
...
The dataset also measures number of plastic bottles, cigarette butts, glass bottles, grocery bags, chip bags and sports balls. The average of the above variables is: ```r mean(merged_df$plastic_bottles)```, ```r mean(merged_df$cigarette_butts)```, ```r mean(merged_df$glass_bottles)```, ```r mean(merged_df$grocery_bags)```, ```r mean(merged_df$chip_bags)```, ```r mean(merged_df$sports_balls)``` respectively. 
...
# Problem 3
First, read the dataset. Then separate the mon variable into variables year month and day. As instructed, these variables need to be in integers. Therefore, use mutate function to convert all values in the variables into integers. After that, convert month into month name using moth.abb function in r. Then, create a president variable 
```{r}
